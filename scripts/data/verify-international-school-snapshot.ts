...
export async function verifyInternationalSchoolSnapshot(
  ...
  const officialIds = new Set(
    packageValue.institutions.map((item) => item.registryExternalId),
  );
  const snapshotIds = new Set(
    packageValue.snapshot.institutions.map((item) => item.registryExternalId),
  );
  const exactCounts =
  ...
  const importBoundary =
    ...
    packageValue.snapshot.institutions.every(
      (item) =>
        item.category === "INTERNATIONAL_SCHOOL" &&
        item.publicationState === "DRAFT",
    );
  const identityComplete = packageValue.institutions.every((institution) =>
    packageValue.evidence.some(
      (evidence) =>
  ...
  const checks = [
    {
      name: "strict schema and reference validation",
      status: validation.status,
    },
    {
      name: "raw file and canonical snapshot checksums",
      status: validation.checksumsValid ? "PASS" : "FAIL",
    },
    {
      name: "official special-access and candidate counts",
      status: exactCounts ? "PASS" : "FAIL",
    },
    {
      name: "official-only draft import boundary",
      status: importBoundary ? "PASS" : "FAIL",
    },
    {
      name: "captured official identity evidence",
...
async function main(): Promise<void> {
  const verification = await verifyInternationalSchoolSnapshot(
    option("dir") ?? DEFAULT_DIRECTORY,
  );
