...
import {
  manifestFileNames,
  type InternationalSchoolManifest,
...
const ndjson = (values: readonly unknown[]) =>
  `${values.map((value) => JSON.stringify(value)).join("\n")}\n`;
const json = (value: unknown) => `${JSON.stringify(value, null, 2)}\n`;
const isiNumber = (value: { registryExternalId: string }) =>
  Number(value.registryExternalId.slice(5));
...
export async function buildInternationalSchoolSnapshot(
  ...
  const snapshot = {
    ...INTERNATIONAL_SCHOOL_IMPORT_SNAPSHOT,
    institutions: [...INTERNATIONAL_SCHOOL_IMPORT_SNAPSHOT.institutions].sort(
      (a, b) => isiNumber(a) - isiNumber(b),
    ),
  };
  const contents = {
    "institutions.ndjson": ndjson(
      [...OFFICIAL_ACTIVE_INTERNATIONAL_SCHOOLS].sort(
        (a, b) => isiNumber(a) - isiNumber(b),
      ),
    ),
    "evidence.ndjson": ndjson(
      [...INTERNATIONAL_SCHOOL_EVIDENCE].sort((a, b) =>
        a.evidenceId.localeCompare(b.evidenceId),
      ),
    ),
    "social-evidence.ndjson": ndjson(
      [...INTERNATIONAL_SCHOOL_SOCIAL_EVIDENCE].sort((a, b) =>
        a.socialEvidenceId.localeCompare(b.socialEvidenceId),
      ),
    ),
    "special-access.ndjson": ndjson(
      [...SPECIAL_ACCESS_RECORDS].sort((a, b) =>
        a.recordId.localeCompare(b.recordId),
      ),
    ),
    "candidates.ndjson": ndjson(
      [...CANDIDATE_SCHOOLS].sort((a, b) =>
        a.recordId.localeCompare(b.recordId),
      ),
    ),
  ...
  await Promise.all(
    manifestFileNames.map((filename) =>
      writeFile(resolve(directory, filename), contents[filename], "utf8"),
    ),
  );
  const manifest: InternationalSchoolManifest = {
    ...
    ) as InternationalSchoolManifest["files"],
...
async function main(): Promise<void> {
  const manifest = await buildInternationalSchoolSnapshot(
    option("output") ?? DEFAULT_OUTPUT,
  );
